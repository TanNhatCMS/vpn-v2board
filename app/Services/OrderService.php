...
class OrderService
{
    ...
    public function setOrderType(User $user): void
    {
        ...
        } elseif ($user->expired_at > time() && $order->plan_id == $user->plan_id) { // User subscription has not expired and the subscription purchased is the same as the current subscription === Renewal
            $order->type = 2;
        } else { // 新购
            $order->type = 1;
        }
    ...
    private function buyByPeriod(Order $order, Plan $plan): void
    {
        ...
        // transfer from one time to loop
    ...
    private function buyByOneTime(Plan $plan): void
    {
        $this->buyByResetTraffic();
        $this->user->transfer_enable = $plan->transfer_enable * 1073741824;
        $this->user->plan_id = $plan->id;
